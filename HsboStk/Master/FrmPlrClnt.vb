...
Public Class FrmPlrClnt
    Private Sub FrmPlrClnt_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Center(Me)
    ...
    Private Sub FillGrid()
        Dim lst = (From s In db.V_ClntCrpArvs Select s.trkClntCrp, s.clntCrpName, s.arivalName, s.clntCrpTel, s.clntCrpInfo, s.trkArival).ToList
        ...
        If CanDelete() = True Then
            If MemEdit Then
                ...
                If Msg = MsgBoxResult.Yes Then
                    Dim tb As New clientCrp
                    tb = (From s In db.clientCrps Where s.trkClntCrp = Val(TxtID.Text) Select s).Single()
                    tb.delClntCrp = True
                    db.SubmitChanges()
                    FillGrid()
    ...
    Public Function CanDelete() As Boolean
        CanDelete = False
        'first step
        Dim lst = (From s In db.aStoreReqs Where s.trkClntCrp = Val(TxtID.Text) _
                                               And s.delAS = 0 Select s).ToList()
        If lst.Count > 0 Then
            Exit Function
        End If
        CanDelete = True
    ...
    Private Function LocAvail() As Boolean
        ...
        LocAvail = False
        Dim tb As New clientCrp
        tb = (From s In db.clientCrps Where s.clntCrpName.Trim Like Trim(TxtName.Text) _
        And s.trkClntCrp <> Val(TxtID.Text) And s.trkArival = Val(LokLoc.EditValue)
    ...
    Private Sub SaveData()
        Dim tb As New clientCrp
        If MemAdd = True Then
            tb.trkClntCrp = NewKey()
            tb.clntCrpName = TxtName.Text
            tb.clntCrpTel = Val(TxtTel.Text)
            tb.trkArival = Val(LokLoc.EditValue)
            tb.clntCrpInfo = TxtInfo.Text
            tb.delClntCrp = False
            db.clientCrps.InsertOnSubmit(tb)
        ElseIf MemEdit = True
            tb = (From s In db.clientCrps Where s.trkClntCrp = Val(TxtID.Text) Select s).Single()
            tb.clntCrpName = TxtName.Text
            tb.clntCrpTel = Val(TxtTel.Text)
    ...
    Function NewKey() As Integer
        Dim max As Integer
        Dim Qrymax = (From trk In db.clientCrps Select trk.trkClntCrp).ToList
    ...
    Private Sub GVDriver_SelectionChanged(sender As Object, e As SelectionChangedEventArgs) Handles GVClient.SelectionChanged
        ...
        If countSelect <> 0 Then
            ...
            TxtID.Text = Val(GVClient.GetRowCellValue(RowInd(0), "trkClntCrp"))
            TxtName.Text = GVClient.GetRowCellValue(RowInd(0), "clntCrpName")
            TxtTel.Text = GVClient.GetRowCellValue(RowInd(0), "clntCrpTel")
            TxtInfo.Text = GVClient.GetRowCellValue(RowInd(0), "clntCrpInfo")

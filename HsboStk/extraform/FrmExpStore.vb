...
Public Class FrmExpStore
        ...
        If CanDelete() = True Then
            If MemEdit Then
                ...
                If Msg = MsgBoxResult.Yes Then
                    Dim tb As New exportStore
                    tb = (From s In db.exportStores Where s.trkExpStore = Val(TxtID.Text) Select s).Single()
                    tb.delSExp = True
                    db.SubmitChanges()
                    ClearForm()
                End If
    ...
    Public Function CanDelete() As Boolean
        CanDelete = False
        Dim lstStr = (From s In db.expStocks Where s.trkExpStore = Val(TxtID.Text) And s.delExpStk = 0 Select s).ToList()
        If lstStr.Count > 0 Then
            Exit Function
        End If
        Dim lstExpSt = (From s In db.goodShps Where s.trkExpLok = Val(TxtID.Text) And s.delGood = 0 Select s).ToList()
    ...
    Private Function CanSave() As Boolean
        ...
        If LocAvail() = False Then
            XtraMessageBox.Show(DevExpress.LookAndFeel.UserLookAndFeel.Default, "عفواً اسم المخزن في منطقة الصادر مسجل مسبقاً", "نقص في الإدخال", MessageBoxButtons.OK, MessageBoxIcon.Hand)
            TxtName.Focus()
            TxtName.SelectAll()
            Exit Function
        End If
    ...
    Private Function LocAvail() As Boolean
        ...
        LocAvail = False
        Dim tb As New exportStore
        tb = (From s In db.exportStores Where s.expStore.Trim Like Trim(TxtName.Text) And s.delSExp = False _
        And s.trkExpStore <> Val(TxtID.Text) And s.trkExpLoc = Val(LokLoc.EditValue)
    ...
    Private Sub SaveData()
        Dim tb As New exportStore
        If MemAdd = True Then
            tb.trkExpStore = NewKey()
            tb.expStore = TxtName.Text
            tb.trkExpLoc = Val(LokLoc.EditValue.ToString())
            tb.delSExp = False
            db.exportStores.InsertOnSubmit(tb)
        ElseIf MemEdit = True
            tb = (From s In db.exportStores Where s.trkExpStore = Val(TxtID.Text) Select s).Single()
            tb.expStore = TxtName.Text
            tb.trkExpLoc = Val(LokLoc.EditValue)
        End If
        db.SubmitChanges()
        Progress()
    End Sub
    Function NewKey() As Integer
        Dim max As Integer
        Dim Qrymax = (From trk In db.exportStores Select trk.trkExpStore).ToList
    ...
    Private Sub BtnSearch_Click(sender As Object, e As EventArgs) Handles BtnSearch.Click
        ...
        ClearForm()
        MemReq = trk
        Dim Mylst As New LstExpStr
        Mylst.ShowDialog()
        If ID <> 0 Then
            TxtID.Text = ID
            Dim tb As New V_ExportStore
            tb = (From s In db.V_ExportStores Where s.trkExpStore = Val(ID) Select s).Single
            TxtName.Text = tb.expStore
            LokLoc.Reset()
            LokLoc.EditValue = tb.trkExpLoc
            LokLoc.Refresh()
            LokLoc.Text = tb.expName
            If Not MemEdit Then
                MemEdit = True
                TxtName.Focus()
            Else

...
#End Region
    ...
    Private Sub NotSaved()
        ...
        End If
        Dim lst = (From s In db.PrdClntRcvPlr(TheDate, Val(TxtJobOrd.Text), Val(PLok), Val(PPeeler)) Select s).ToList
        ...
        While i < count
            ...
            If StrUn <> "" And curPrd <> 0 Then
                ...
                If Val(GVRecvDet.GetRowCellValue(i, "trkRecDet")) <> 0 Then
                    Clnt = Val(PClient)
                    'tbShip = (From s In db.recvClntDets Where s.trkRecvClntDet = Val(GVRecvDet.GetRowCellValue(i, "trkRecDet"))
                    '          Select s).SingleOrDefault()
                    '*******************
                    tbRcvClnt = (From s In db.V_RecvClntDets Where s.trkRecvClntDet = Val(GVRecvDet.GetRowCellValue(i, "trkRecDet")) _
                                                            And s.trkPeeler = Val(PPeeler) And s.trkToPrd = Val(TxtJobOrd.Text) _
                                                                 And s.trkClntCrp = Clnt _
            ...
            If StrUn <> "" And prdName <> "" Then
                ...
                If TheId <> 0 Then
                    tbRcvClnt = (From s In db.V_RecvClntDets Where s.trkRecvClntDet = TheId _
                                                            And s.trkPeeler = Val(PPeeler) And s.trkToPrd = Val(TxtJobOrd.Text) _
                                                                 And s.trkClntCrp = Val(PClient) _
